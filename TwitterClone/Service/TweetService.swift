...
struct TweetService {
    ...
    func uploadTweet(caption: String, completion:@escaping(Bool) -> Void) {
        ...
        Firestore.firestore().collection("tweets").document().setData(data) { error in
            if let error = error {
                print("DEBUG: Error uploading tweet with error \(error.localizedDescription)")
                completion(false)
                return
            }
            completion(true)
    ...
    func fetchTweets(forUid: String, completion:@escaping([Tweet]) -> Void) {
        Firestore.firestore().collection("tweets")
            .whereField("uid", isEqualTo: forUid)
            .getDocuments { snapshot, _ in
                guard let documents = snapshot?.documents else { return }
                let tweets = documents.compactMap({try? $0.data(as: Tweet.self)})
